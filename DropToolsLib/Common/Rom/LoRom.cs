...
public class LoRom : Rom {
	public override PCAddress ToPCAddress(int address) {
		if ((0x00ffff & address) < 0x8000) {
			throw new ArgumentException($"lower word of {nameof(address)} must be at least 0x8000", nameof(address));
		}
	...
	public override SNESAddress ToSNESAddress(int address) {
		var snesAddress = (address / 0x8000 * 0x10000) + (address % 0x8000) + 0x8000;
		return new SNESAddress(snesAddress);
	}
